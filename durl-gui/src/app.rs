...
pub struct DurlApp {
    ...
    // Shared log output for all tasks
    logs: Vec<String>,
...
impl DurlApp {
    ...
    fn setup_fonts(ctx: &egui::Context) {
        let candidates: &[&str] = &[
            ...
            // Linux – WenQuanYi (apt: fonts-wqy-microhei / fonts-wqy-zenhei)
            "/usr/share/fonts/truetype/wqy/wqy-microhei.ttc",
            "/usr/share/fonts/truetype/wqy/wqy-zenhei.ttc",
            // Linux – Arphic (apt: fonts-arphic-uming / fonts-arphic-ukai)
        ...
        if let Ok(mut buf) = self.log_buffer.lock() {
            ...
            for entry in entries {
                ...
                self.logs.push(format!(
                    "[{}] [{}] {}",
                    entry.time, level_tag, entry.message
                ));
            }
        ...
        for task in &mut self.tasks {
            ...
            if let Some(rx) = &task.sha256_rx {
                if let Ok(hash) = rx.try_recv() {
                    ...
                    self.logs.push(format!("[{}] SHA256: {}", now_str(), hash));
                    task.sha256_rx = None;
                    self.dirty = true;
                }
            }
                ...
                if let Ok(result) = rx.try_recv() {
                    match result {
                        ...
                        Err(e) => {
                            ...
                            self.logs.push(format!("[{}] Failed: {}", now_str(), e));
                        }
                    }
                    task.receiver = None;
                    self.dirty = true;
            ...
            if let Some(df) = &task.download {
                    ...
                    && task.status != TaskStatus::Error
                {
                    if status.is_error() {
                        ...
                        self.logs
                            .push(format!("[{}] Completed: {}", now_str(), task.file_path));
    ...
    fn start_download(&mut self) {
        ...
        let custom_name = if self.new_filename.trim().is_empty() {
            None
        } else {
            Some(self.new_filename.trim().to_string())
        };
        let cookies = if self.new_cookies.trim().is_empty() {
            None
        } else {
            Some(self.new_cookies.trim().to_string())
        };
        ...
        self.rt.spawn(async move {
            let result = DownloadFile::start_download(
                url_clone,
                save_path,
                task_count,
                1024 * 1024,
        ...
        let display_name = if !self.new_filename.trim().is_empty() {
            self.new_filename.trim().to_string()
        } else {
        ...
        self.tasks.push(DownloadTask {
            ...
            save_dir: self.new_save_path.clone(),
            file_size: 0,
            downloaded: 0,
            speed: 0,
            progress: 0.0,
        ...
        self.logs
            .push(format!("[{}] Preparing: {}", now_str(), url));
    ...
    fn handle_action(&mut self, action: Action) {
        match action {
            ...
            Action::Pause(id) => {
                ...
                self.logs.push(format!("[{}] Paused", now_str()));
                self.mark_dirty();
            }
            Action::Resume(id) => {
                if let Some(t) = self.tasks.iter_mut().find(|t| t.id == id) {
                    if let Some(df) = &t.download {
                        ...
                        self.logs.push(format!("[{}] Resumed", now_str()));
                    } else {
                        let url = t.url.clone();
                        ...
                        t.error_msg = None;
                        self.logs.push(format!("[{}] Restarting", now_str()));
                    }
                }
                self.mark_dirty();
            }
            ...
            Action::Redownload(id) => {
                if let Some(t) = self.tasks.iter_mut().find(|t| t.id == id) {
                    ...
                    t.sha256 = None;
                    t.sha256_rx = None;
                    self.logs.push(format!("[{}] Re-downloading", now_str()));
                }
                self.mark_dirty();
            }
        }
        ...
        ui.horizontal(|ui| {
            ...
            if ui
                .add_sized(
                    btn_size,
                    egui::Button::new(
                        RichText::new(self.lang.get("toolbar.new_download")).size(13.0),
                    ),
                )
                .clicked()
            {
            ...
            let sel_status = self
                .selected_task_id
                .and_then(|id| self.tasks.iter().find(|t| t.id == id).map(|t| t.status));
            ...
            if ui
                .add_enabled(
                    can_pause,
                    egui::Button::new(RichText::new(self.lang.get("toolbar.pause")).size(13.0))
                        .min_size(btn_size),
                )
                .clicked()
            {
                if let Some(id) = self.selected_task_id {
                    self.handle_action(Action::Pause(id));
                }
            }
            if ui
                .add_enabled(
                    can_resume,
                    egui::Button::new(RichText::new(self.lang.get("toolbar.resume")).size(13.0))
                        .min_size(btn_size),
                )
                .clicked()
            {
                if let Some(id) = self.selected_task_id {
                    self.handle_action(Action::Resume(id));
                }
            }
            if ui
                .add_enabled(
                    has_selected,
                    egui::Button::new(RichText::new(self.lang.get("toolbar.delete")).size(13.0))
                        .min_size(btn_size),
                )
                .clicked()
            {
                if let Some(id) = self.selected_task_id {
                    self.handle_action(Action::Delete(id));
                }
            }
            ui.separator();
            ...
            if ui
                .add_sized(
                    btn_size,
                    egui::Button::new(RichText::new(self.lang.get("toolbar.settings")).size(13.0)),
                )
                .clicked()
            {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                if active > 0 {
                    ...
                    let text = self.lang.get_fmt(
                        "toolbar.active_speed",
                        &[("active", &active.to_string()), ("speed", &speed_str)],
                    );
    ...
    fn render_sidebar(&mut self, ui: &mut egui::Ui) {
        ...
        ui.vertical(|ui| {
            ui.label(
                RichText::new(self.lang.get("sidebar.my_downloads"))
                    .size(16.0)
                    .strong(),
            );
            ui.add_space(12.0);
            ...
            let items: [(Filter, String, usize); 3] = [
                (
                    Filter::All,
                    self.lang.get("sidebar.all_tasks").to_string(),
                    self.tasks.len(),
                ),
                (
                    Filter::Downloading,
                    self.lang.get("sidebar.downloading").to_string(),
                    self.count_downloading(),
                ),
                (
                    Filter::Completed,
                    self.lang.get("sidebar.completed").to_string(),
    ...
    fn render_table(&mut self, ui: &mut egui::Ui) {
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ui.add_sized(
                        [COL_STATUS, 20.0],
                        egui::Label::new(
                            RichText::new(self.lang.get("table.col_status"))
                                .size(12.0)
                                .strong(),
                        ),
                    );
                    ui.add_sized(
                        [col_name, 20.0],
                        egui::Label::new(
                            RichText::new(self.lang.get("table.col_filename"))
                                .size(12.0)
                                .strong(),
                        ),
                    );
                    ui.add_sized(
                        [COL_SIZE, 20.0],
                        egui::Label::new(
                            RichText::new(self.lang.get("table.col_filesize"))
                                .size(12.0)
                                .strong(),
                        ),
                    );
                    ui.add_sized(
                        [COL_PROGRESS, 20.0],
                        egui::Label::new(
                            RichText::new(self.lang.get("table.col_progress"))
                                .size(12.0)
                                .strong(),
                        ),
                    );
                    ui.add_sized(
                        [COL_SPEED, 20.0],
                        egui::Label::new(
                            RichText::new(self.lang.get("table.col_speed"))
                                .size(12.0)
                                .strong(),
                        ),
                    );
                    ui.add_sized(
                        [COL_REMAIN, 20.0],
                        egui::Label::new(
                            RichText::new(self.lang.get("table.col_remaining"))
                                .size(12.0)
                                .strong(),
                        ),
                    );
                    ui.add_sized(
                        [COL_ELAPSED, 20.0],
                        egui::Label::new(
                            RichText::new(self.lang.get("table.col_elapsed"))
                                .size(12.0)
                                .strong(),
                        ),
                    );
                    ui.add_sized(
                        [COL_TYPE, 20.0],
                        egui::Label::new(
                            RichText::new(self.lang.get("table.col_filetype"))
                                .size(12.0)
                                .strong(),
                        ),
                    );
                    ui.add_sized(
                        [COL_SHA256, 20.0],
                        egui::Label::new(
                            RichText::new(self.lang.get("table.col_sha256"))
                                .size(12.0)
                                .strong(),
                        ),
                    );
        ...
        egui::ScrollArea::vertical()
            .auto_shrink([false; 2])
            .show(ui, |ui| {
                let indices: Vec<usize> = (0..self.tasks.len())
                    .filter(|&i| self.task_visible(&self.tasks[i]))
                    .collect();
                    ...
                    let resp = egui::Frame::none()
                        ...
                        .show(ui, |ui| {
                            ui.horizontal(|ui| {
                                ...
                                ui.add_sized(
                                    [COL_STATUS, ROW_H],
                                    egui::Label::new(RichText::new(icon).size(14.0).color(color)),
                                );
                                ui.add_sized(
                                    [col_name, ROW_H],
                                    egui::Label::new(RichText::new(&filename).size(12.5))
                                        .truncate(),
                                );
                                let size_str = if file_size > 0 {
                                    format_size(file_size, BINARY).to_string()
                                } else {
                                    "—".into()
                                };
                                ui.add_sized(
                                    [COL_SIZE, ROW_H],
                                    egui::Label::new(RichText::new(size_str).size(12.0)),
                                );
                                ui.allocate_ui(Vec2::new(COL_PROGRESS, ROW_H), |ui| {
                                    ui.centered_and_justified(|ui| {
                                        ...
                                        let bar = egui::ProgressBar::new(pct)
                                            ...
                                            .fill(if status == TaskStatus::Completed {
                                                GREEN
                                            } else {
                                                BLUE_PRIMARY
                                            });
                                ...
                                ui.add_sized(
                                    [COL_SPEED, ROW_H],
                                    egui::Label::new(RichText::new(format_speed(speed)).size(12.0)),
                                );
                                ui.add_sized(
                                    [COL_REMAIN, ROW_H],
                                    egui::Label::new(
                                        RichText::new(remaining_str(file_size, downloaded, speed))
                                            .size(12.0),
                                    ),
                                );
                                ui.add_sized(
                                    [COL_ELAPSED, ROW_H],
                                    egui::Label::new(RichText::new(format_dur(elapsed)).size(12.0)),
                                );
                                ui.add_sized(
                                    [COL_TYPE, ROW_H],
                                    egui::Label::new(
                                        RichText::new(self.lang.get(file_type_key(&filename)))
                                            .size(12.0),
                                    ),
                                );
                                ui.add_sized(
                                    ...
                                    egui::Label::new(
                                        RichText::new(
                                            self.tasks[task_idx].sha256.as_deref().unwrap_or("—"),
                                        )
                                        .size(12.0),
                                    ),
                                );
    ...
    fn render_log_panel(&self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ui.label(
                RichText::new(self.lang.get("log_panel.title"))
                    .size(12.0)
                    .strong(),
            );
        });
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                if self.logs.is_empty() {
                    ui.label(
                        RichText::new(self.lang.get("log_panel.empty"))
                            .size(12.0)
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if ui
                            .add_enabled(
                                !self.new_url.trim().is_empty(),
                                egui::Button::new(RichText::new(&lbl_start).color(Color32::WHITE))
                                    .fill(BLUE_PRIMARY),
                            )
                            .clicked()
                        {
    ...
    fn render_settings_dialog(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new(&title)
            ...
            .show(ctx, |ui| {
                egui::Grid::new("settings_grid")
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.horizontal(|ui| {
                            ui.add_sized(
                                [300.0, 24.0],
                                egui::TextEdit::singleline(&mut self.user_config.default_save_path),
                            );
                            if ui.button(&lbl_browse).clicked() {
                                if let Some(dir) = rfd::FileDialog::new()
                                    .set_directory(&self.user_config.default_save_path)
                                    .pick_folder()
                                {
                        ...
                        if ui
                            .add_sized([80.0, 24.0], egui::TextEdit::singleline(&mut tc_str))
                            .changed()
                        {
                            if let Ok(v) = tc_str.parse::<u64>() {
                                self.user_config.default_task_count = v;
                            }
                        }
                        ui.end_row();
                        ...
                        ui.horizontal(|ui| {
                            for (lang_id, display) in &available_langs {
                                if ui
                                    .selectable_label(
                                        self.user_config.language == *lang_id,
                                        display,
                                    )
                                    .clicked()
                                {
                        ...
                        ui.horizontal(|ui| {
                            for &level in LOG_LEVELS {
                                if ui
                                    .selectable_label(
                                        self.user_config.log_level.eq_ignore_ascii_case(level),
                                        level,
                                    )
                                    .clicked()
                                {
                                    self.user_config.log_level = level.to_string();
                                    crate::gui_logger::set_log_level(
                                        self.user_config.log_level_filter(),
                                    );
                                }
                            }
                        });
                ...
                ui.add_space(2.0);
                ui.label(
                    RichText::new(&lbl_lang_dir_hint)
                        .size(11.0)
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui
                        .add(
                            egui::Button::new(RichText::new(&lbl_ok).color(Color32::WHITE))
                                .fill(BLUE_PRIMARY),
                        )
                        .clicked()
                    {
