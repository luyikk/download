...
void durl_start(DownloadHandler *handler,
                const char *url,
                const char *path,
                uint64_t task,
                uint64_t block);
...
bool durl_is_downloading(const DownloadHandler *handler);

bool durl_is_downloading_finish(const DownloadHandler *handler);
...
void durl_get_error_str(const DownloadHandler *handler, char *msg);
