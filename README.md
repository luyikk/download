...
| GUI with task list, progress, SHA256, i18n, browser extension | GUI 带任务列表、进度、SHA256 校验、多语言、浏览器扩展 |
---
## Install / 安装
...
- Shared log panel for all tasks
  所有任务共享的日志面板
- i18n: built-in zh-CN, en-US, ja-JP, ru-RU, fr-FR, de-DE — user-extensible via TOML files
  多语言：内置中文/英文/日文/俄文/法文/德文，可在配置目录自定义 TOML 语言文件
- Config stored in OS app-config directory (Windows: `%APPDATA%/durl-gui/`)
  配置存储在系统配置目录（Windows：`%APPDATA%/durl-gui/`）
- CJK font auto-detection on Windows, macOS, Linux
  自动检测 Windows/macOS/Linux 系统 CJK 字体
- Browser extension support (Chrome & Edge) — intercepts browser downloads and sends to durl-gui
  内置浏览器扩展支持（Chrome & Edge）——拦截浏览器下载并直接发送到 durl-gui
**Browser Extension Install / 浏览器扩展安装：**
1. Open **Settings → Install for Chrome / Install for Edge** — browser opens automatically
   打开**设置 → 安装到 Chrome / 安装到 Edge**——浏览器会自动打开
2. Copy the URL shown in the dialog and paste it into the browser address bar
   将对话框中显示的地址复制后粘贴到浏览器地址栏
   - Chrome: `chrome://extensions`
   - Edge: `edge://extensions`
3. Enable **Developer mode** (Chrome: top-right toggle; Edge: bottom-left toggle)
   开启**开发者模式**（Chrome：右上角开关；Edge：左下角开关）
4. Click **Load unpacked** and select the folder shown in the dialog
   点击**加载已解压的扩展程序**，选择对话框中显示的文件夹
...
```toml
# Correct / 正确
download-lib = { path = "../download-lib", version = "0.2.9" }
